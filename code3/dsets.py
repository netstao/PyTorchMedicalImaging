...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    candidateInfo_list = []
    with open('F:/BaiduNetdiskDownload/luna/annotations_with_malignancy.csv', "r") as f:
...
class Ct:
    def __init__(self, series_uid):
        ...
        if not mhd_path:
            raise ValueError(f"No .mhd file found for series_uid: {series_uid}")
        if len(mhd_path)<=0:
            raise ValueError(f"mhd_path len is zero for series_uid: {series_uid}")
